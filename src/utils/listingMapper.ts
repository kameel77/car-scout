...
export function mapBackendListingToFrontend(backendListing: any): Listing | null {
    try {
        ...
        return {
            ...
            price_pln: backendListing.pricePln || 0,
            price_display: backendListing.priceDisplay || `${backendListing.pricePln || 0} PLN`,
            production_year: backendListing.productionYear || 0,
            mileage_km: backendListing.mileageKm || 0,
            ...
            image_urls: backendListing.imageUrls && backendListing.imageUrls.length > 0
                ? backendListing.imageUrls.map((url: string) => url.replace(/\.(jpg|jpeg|png)$/i, '.webp'))
                : ['/motolia-placeholder.png'],
