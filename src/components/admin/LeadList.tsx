...
export function LeadList() {
    ...
    const [selectedLeadType, setSelectedLeadType] = React.useState<string>('all');
    ...
    const { data, isLoading, isFetching, isError } = useQuery({
        queryKey: ['leads', token, selectedLeadType],
        queryFn: async () => {
            if (!token) throw new Error('Brak tokenu');
            const typeFilter = selectedLeadType === 'all' ? undefined : selectedLeadType;
            return leadsApi.getLeads(token, typeFilter);
        },
        enabled: !!token
    });
    ...
    const mappedLeads = React.useMemo<Lead[]>(() => {
        ...
        return (data?.leads || []).map((lead: any): Lead => {
            ...
            return {
                ...
                reference_number: lead.referenceNumber,
    ...
    const filteredLeads = React.useMemo(() => {
        ...
        return mappedLeads.filter(lead =>
            ...
            (lead.reference_number || '').toLowerCase().includes(q) ||
            (lead.metadata?.companyName || '').toLowerCase().includes(q) ||
            (lead.metadata?.companyNip || '').toLowerCase().includes(q) ||
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Lead Type Filter Bar */}
            <div className="flex flex-wrap items-center gap-2">
                {[
                    { id: 'all', label: 'Wszystkie' },
                    { id: 'employer_b2b', label: 'Benefivo B2B' },
                    { id: 'sale', label: 'Motolia (Sprzedaż)' },
                    { id: 'rental', label: 'Wynajem' },
                    { id: 'waitlist', label: 'Lista oczekujących' },
                    { id: 'quick_contact', label: 'Szybki kontakt' },
                ].map((tab) => {
                    const isActive = selectedLeadType === tab.id;
                    return (
                        <Button
                            key={tab.id}
                            type="button"
                            ...
                            size="sm"
                            className={isActive && tab.id === 'employer_b2b' ? 'bg-[#0f2d1e] hover:bg-[#1a4a32] text-[#F7F8F2]' : ''}
                            onClick={() => setSelectedLeadType(tab.id)}
                        >
                            {tab.label}
                        </Button>
                    );
                                    ...
                                    {lead.lead_type === 'employer_b2b' ? (
                                        <div className="flex flex-col gap-1 items-start">
                                            ...
                                            {lead.reference_number && (
                                                <span className="text-[10px] font-mono bg-stone-100 px-1.5 py-0.5 rounded text-stone-600 w-fit">
                                                    {lead.reference_number}
                                                </span>
                                            )}
                                        </div>
                                ...
                                <TableCell className="align-top text-left">
                                    <div className="flex flex-col gap-1.5 items-start">
                                        <div className="flex flex-wrap items-center gap-1.5">
                                            ...
                                            {lead.lead_type === 'employer_b2b' && (
                                                <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold bg-[#0f2d1e] text-[#F7F8F2]">
                                                    Benefivo B2B
                                                </span>
                                            )}
                                        </div>
                    ...
                    {selectedLead && (
                        <div className="grid gap-6 py-4">
                            <div className="grid grid-cols-2 gap-8">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex items-center gap-2">
                                            ...
                                            {selectedLead.lead_type === 'employer_b2b' && (
                                                <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold bg-[#0f2d1e] text-[#F7F8F2]">
                                                    Benefivo B2B
                                                </span>
                                            )}
                                ...
                                {/* Offer Info or B2B Company Info */}
                                {selectedLead.lead_type === 'employer_b2b' ? (
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between border-b pb-2">
                                            <h4 className="text-xs font-bold uppercase tracking-widest text-[#0f2d1e] flex items-center gap-1.5">
                                                <Building2 className="h-3.5 w-3.5 text-[#0f2d1e]" /> Firma (Benefivo B2B)
                                            </h4>
                                            {selectedLead.reference_number && (
                                                <span className="text-[11px] font-mono bg-stone-100 text-stone-700 px-2 py-0.5 rounded font-semibold">
                                                    {selectedLead.reference_number}
                                                </span>
                                            )}
                                        </div>
                                        {selectedLead.metadata ? (
                                            <div className="space-y-3 bg-stone-50/80 p-3.5 rounded-xl border border-stone-200/80">
                                                <div>
                                                    <p className="text-[10px] uppercase text-stone-500 font-bold tracking-wider">Nazwa firmy</p>
                                                    <p className="font-bold text-base text-stone-900">{selectedLead.metadata.companyName || 'Brak danych'}</p>
                                                </div>
                                                {selectedLead.metadata.companyNip && (
                                                    <div>
                                                        <p className="text-[10px] uppercase text-stone-500 font-bold tracking-wider">NIP</p>
                                                        <div className="flex items-center gap-2 mt-0.5">
                                                            <span className="font-mono text-sm font-semibold text-stone-800 bg-white px-2 py-0.5 rounded border border-stone-200">
                                                                {selectedLead.metadata.companyNip}
                                                            </span>
                                                            <Button
                                                                type="button"
                                                                ...
                                                                onClick={() => {
                                                                    navigator.clipboard.writeText(selectedLead.metadata?.companyNip || '');
                                                                    toast.success('Skopiowano NIP do schowka');
                                                                }}
                                                            >
                                                ...
                                                <div className="grid grid-cols-2 gap-3 pt-1 border-t border-stone-200/60">
                                                    {selectedLead.metadata.teamSize && (
                                                        <div>
                                                            <p className="text-[10px] uppercase text-stone-500 font-bold tracking-wider">Wielkość zespołu</p>
                                                            <p className="text-sm font-semibold text-stone-800">{selectedLead.metadata.teamSize} osób</p>
                                                        </div>
                                                    )}
                                                    {selectedLead.metadata.benefitModel && (
                                                        <div>
                                                            <p className="text-[10px] uppercase text-stone-500 font-bold tracking-wider">Model benefitu</p>
                                                            <p className="text-sm font-semibold text-stone-800">{selectedLead.metadata.benefitModel}</p>
                                                        </div>
                                                    )}
                                                </div>
                                                <div className="pt-1 border-t border-stone-200/60 text-[11px] text-stone-500">
                                                    <span>Źródło: </span>
                                                    <span className="font-mono font-medium text-stone-700">
                                                        {selectedLead.traffic_source === 'benefivo_b2b' ? 'benefivo.pl/dla-firm' : (selectedLead.traffic_source || 'benefivo.pl')}
                                                    </span>
                                                </div>
                                            </div>
                                        ) : (
                                            <div className="p-3 bg-stone-50 rounded-lg text-xs text-stone-600">
                                                Zgłoszenie B2B bez zapisanych metadanych strukturalnych.
                                            </div>
                                        )}
                                    </div>
                                ) : (
